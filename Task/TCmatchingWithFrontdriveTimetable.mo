...
model TCmatchingWithFrontdriveTimetable
  parameter Real ne[:,:] = [900, 1903; 1000, 2138; 1100, 2281; 1200, 2282; 1300, 2281; 1500, 2282; 1700, 2041; 1900, 1826; 2050, 0] "Pump speed, rpm";
  parameter Real frontDrive[2] = {1.0, 1.0};
  ...
  Real np[size(ne, 1),size(ne, 2)] "Pump speed, rpm";
  ...
  Modelica.Blocks.Sources.TimeTable timeTable(table = [0:size(lambdaTable, 1) - 1, 1:size(lambdaTable, 1)]) annotation (
    Placement(visible = true, transformation(origin = {-30, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
protected 
  Integer j;
equation 
  np = [ne[:,1] / frontDrive[1], ne[:,2] * frontDrive[1] * frontDrive[2]];
  j = timeTable.y;
  Tp = oilDensity * 9.8 * lambdaTable[j,4] * np[:,1] .^ 2 * diameter ^ 5 / 10 ^ 6;
  npCowork = Blocks.InterpolateVector(0, [Tp - np[:,2], np[:,1]]);
  ntCowork = npCowork * lambdaTable[j,1];
  TpCowork = oilDensity * 9.8 * lambdaTable[j,4] * npCowork ^ 2 * diameter ^ 5 / 10 ^ 6;
  TtCowork = TpCowork * lambdaTable[j,2];
end TCmatchingWithFrontdriveTimetable;